...
export const fetchMovies = (text) => async (dispatch, getState) => {
    ...
    try{
        const response = await Axios.get(`https://www.omdbapi.com/?s=${text}&type=movie&apikey=${process.env.REACT_APP_OMDB_KEY}`);
...
export const fetchPlot = (text) => async (dispatch, getState) => {
    ...
    try{
        const response = await Axios.get(`https://www.omdbapi.com/?i=${text}&apikey=${process.env.REACT_APP_OMDB_KEY}&Plot=full`);
